...
(function () {
  ...
  SNS.init = function (options) {
    ...
    // 2. Setup Leaflet Map if map container exists
  ...
  SNS.renderResults = function (results) {
    ...
    results.forEach((item, idx) => {
      ...
      // 10 Nearby Essentials HTML if present
      let essentialsHtml = '';
      ...
      // Add Leaflet Map Marker with custom icon
      if (markersLayer && item.latitude && item.longitude) {
        ...
        marker.bindPopup(`
          <div style="font-family:sans-serif;padding:6px;min-width:180px;">
            <strong style="font-size:14px;color:#1e293b;">${icon} ${_esc(item.name)}</strong><br>
